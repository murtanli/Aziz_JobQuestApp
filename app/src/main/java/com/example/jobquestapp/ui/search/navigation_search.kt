package com.example.jobquestapp.ui.search
...
class navigation_search : Fragment() {
    ...
    companion object {
        fun newInstance() = navigation_search()
    }

    private lateinit var viewModel: NavigationSearchViewModel
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_navigation_search, container, false)
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this).get(NavigationSearchViewModel::class.java)
        val all_data = api_resource()


        lifecycleScope.launch {
            try {
                val vacancyList = all_data.getAllVacancy()
                vacancyList?.let { list ->
                    for (vacancyItem in list) {
                        Log.d("VacancyItem", "Title: ${vacancyItem.vacancy_name}")

                        // Создайте блок вакансии и добавьте его в контейнер
                        val vacancyBlock = createVacancyBlock(vacancyItem.vacancy_name, vacancyItem.id, vacancyItem.vacancy_title, vacancyItem.vacancy_salary, vacancyItem.date_publish, vacancyItem.company_name, vacancyItem.company_city)
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun createVacancyBlock(title: String, id: Int, text: String, salary: String, date: String, company_name: String, company_city: String): LinearLayout {
        ...
        val year = date.year
        val monthIndex = date.monthValue
        ...
        //Стилизация кнопки
        val vacancy_button = Button(requireContext())
        vacancy_button.text = "Отклик"
        val backgroundButton = ContextCompat.getDrawable(requireContext(), R.drawable.rounded_button)
        vacancy_button.background = backgroundButton
        ...
        vacancy_button.setOnClickListener {
            it.animate()
                .scaleX(0.7f)
                .scaleY(0.7f)
                .setDuration(300)
                        ...
                        if (log == "false"){
                            ...
                            lifecycleScope.launch(Dispatchers.IO) {
                                try {
                                    ...
                                    val result = data.create_applications(profile_id, id)
        ...
        VacancyBlock.addView(vacancy_button)
